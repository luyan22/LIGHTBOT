...
struct operation{//执行模块 
	int xx[4] = {0, 1, 0, -1};//前进时用于确定具体坐标变化 
	int yy[4] = {-1 , 0, 1, 0};//上右下左 
...
//	}
	...
	int Proc_run(int id){//以一个过程为单位  输入过程编号 
		//cout << id << endl;
		int ans = 0;
		for(int j = 0; j < game.opseq.procs[id].count; j++){
			...
			if(game.opseq.procs[id].ops[j] == TL){//顺时针旋转 右转加一 
				...
				if(new_x < 0 || new_x > game.map_init.col //有待讨论 
...
//		}
	...
	void Run(){//执行指令序列 
		...
		cout << "Step(s) used: " << rs.steps <<endl; //当前花费的步数 
		//继续打印信息 
		if(rs.result == LIGHT){
